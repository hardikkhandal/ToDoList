...
const page = () => {
  ...
  // let rendercompleted = <h2 className="text-white">No task completed</h2>
  // console.log(completed.length)
  // if (completed.length > 0) {
  //   rendercompleted = completed.map((t, i) => {
  //     console.log(t.title)
  //     return (
  //       <li key={i} className="flex items-center justify-between">
  //         <div className="flex justify-between mb-5">
  //           <h5 className="text-2xl text-white font-semibold">{t.title}</h5>
  //           <h6 className="text-lg text-white font-medium">{t.desc}</h6>
  //         </div>
  ...
  return (
    <>
      ...
      <form onSubmit={submitHandler}>
        <input
          type="text"
          className="text-2xl m-5 border-2 border-zinc-800 px-4 py-2"
